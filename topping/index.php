...
if ($action == 'list_toppings') {
    try {
        ...
        $toppingwithoutmeat = get_toppings_withoutmeat($db);
        $toppingswithmeat = get_toppings_withmeat($db);
    ...
    $is_meat_id = filter_input(INPUT_POST, 'is_meat_id', FILTER_VALIDATE_INT);
    if ($topping_id == NULL || $topping_id == FALSE) {
        $e = "Missing or incorrect topping.";
        include('../../errors/error.php');
    } else {
        try {
            delete_topping($db, $topping_id, $is_meat_id);
        } catch (Exception $e) {
            include('../../errors/error.php');
            exit();
        }
    ...
    if ($topping_name == NULL || $topping_name == FALSE) {
        ...
        header("Location: .?topping_name = $topping_name");
    }
}
